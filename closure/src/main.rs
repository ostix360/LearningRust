...
fn generate_exercise(intensity: u32, rand: u32) {
    ...
    if intensity < 25 {
        println!("{} pompes", slow_closure(intensity));
        println!("puis {} abdominaux", slow_closure(intensity));
    }else{
        if rand == 3 {
            println!("take a break today!");
        }else{
            println!("Run during {} mins", slow_closure(intensity));
        }
    }
